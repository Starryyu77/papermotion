...
- [ ] `examples/attention/production_manifest.json` is the current project-level registry.
- [ ] `examples/attention/enriched_scene_spec.json` is the current scene-level source of truth.
- [ ] Scene timing, Manim, TTS, Music, PixVerse, Assembly, and QA fields are aligned in the enriched scene spec.
...
- [ ] Scene Spec Inspector shows the active scene's timing, layers, audio cues, PixVerse cue, assembly state, and QA checks.
- [ ] Assembly Monitor makes Manim exact layer and PixVerse cinematic layer status visible.
...
- [ ] Every generated PixVerse scene also has a matching `enriched_scene_spec.scenes[*].pixverse` entry.
