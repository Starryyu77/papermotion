...
Project-plan fusion update: use `docs/project-plan-fusion.md` for the merged Formula2Video/PaperMotion architecture. The current contract split is `production_manifest.json` as the project-level registry and `enriched_scene_spec.json` as the executable scene-level single source of truth.
...
3. Create/update `examples/attention/production_manifest.json` and `examples/attention/enriched_scene_spec.json`.
4. MTC mode writes intent, prerequisite graph, curriculum plan, teaching script beats, rhythm map, and visual asset requests.
5. Storyboard and Scene Spec agents write scene plans and precise enriched scene specs.
...
- Keep project-level status in `production_manifest.json`; scene-level execution details go into `enriched_scene_spec.json`.
...
- Draft the first TTS/music cue slots for slowdown/reveal moments.
...
Person A:
- Generate or refine `enriched_scene_spec.json` before code.
...
- Fill the voiceover/music/SFX timing plan from `enriched_scene_spec.scenes[*].tts` and `music_cue`.
...
2. Teaching script / storyboard:
   ...
   - Output: script beats, music/voice timing cues, visual asset requests, storyboard scenes.
   - Must include 5-7 scenes with duration, narration, visuals, and PixVerse/Manim classification.

3. Scene specs / Manim:
   - Input: `enriched_scene_spec.json` and optional `scene_specs/*.json`.
